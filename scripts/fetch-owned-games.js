...
async function main() {
  ...
  if (!STEAM_API_KEY || !STEAM_ID64) {
    console.error(
      'Missing STEAM_API_KEY or STEAM_ID64. Add them to .env.local or your environment.'
    );
    process.exit(1);
  }
  ...
  if (!games.length) {
    console.error(
      '[steam:games] No games returned. If your profile or game details are private, consider making Game details public, or use the sample file:'
    );
  ...
  console.error(
    `[steam:games] Wrote ${games.length} games (reported count: ${count}) to ${outFile}`
  );
}
